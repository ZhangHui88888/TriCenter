...
CREATE TABLE enterprise (
    ...
    enterprise_type VARCHAR(20) NOT NULL COMMENT '企业类型（9类预设）',
...
# 各阶段数量分布见看板 GET /api/dashboard/funnel（独立 /api/funnel/* 分析接口已移除）
...
```
登录 → 查询 user_cities
  ├─ 仅一个城市：签发含 currentCityId 的正式 JWT
  └─ 多个城市：签发待选 JWT → POST /api/auth/select-city → 正式 JWT

业务请求 → JWT 过滤器重新校验用户状态和当前城市授权
         → CityContext 提供可信 currentCityId
         → Service/Mapper 显式追加 city_id 条件
```
...
| 角色 | 权限说明 |
|-----|---------|
| admin | 用户与城市授权、字典维护及全部业务操作 |
| manager | 企业增删改、批量、导入导出及跟进管理；不能管理用户和字典 |
| user | 查看和编辑已有企业、管理跟进；不能新增、删除、批量、导入或导出 |

权限使用 `user:manage`、`dictionary:manage`、`enterprise:*`、`followup:manage`
等 authority 在后端方法级校验，前端仅负责同步隐藏菜单和按钮，不能替代后端鉴权。

### 6.3 多城市数据隔离

- 使用单数据库逻辑隔离，`enterprises.city_id` 是业务数据的城市边界。
- 联系人、产品、专利、需求、跟进、合作服务、报告和附件通过
  `enterprise_id → enterprises.city_id` 判断归属，不重复保存 `city_id`。
- 每个正式登录会话只有一个 `currentCityId`，城市只能来自 JWT 对应账号的
  `user_cities` 授权；普通业务请求不接受客户端指定 `city_id`。
- 多城市用户必须先选城，并可在顶栏切换；不提供集团汇总视角。
- 看板缓存键使用 `dashboard:{cityId}:...`，写操作只清理当前城市。
- 服务商、行业分类、系统字典和标准需求属于集团共享基础资料。
- Booking API Key 使用配置的城市代码建立系统上下文，默认固定为
  `changzhou`；仅允许企业创建/更新和数据字典读取端点，不能访问服务商、
  跟进、报告、附件或管理接口。
- 默认管理员的常州授权仅在首次创建账号时写入；应用重启不会恢复后台已经
  撤销的城市授权。
- 合作服务附件下载必须携带服务记录 ID，校验记录所属企业城市后才可读取；
  新文件按城市代码目录保存。
